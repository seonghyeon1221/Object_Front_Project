...
class NextPage extends StatefulWidget {
  ...
  @override
  _NextPageState createState() => _NextPageState(categorysecond);
}

class _NextPageState extends State<NextPage> {
  List<String> categorysecond = [];

  _NextPageState(List<String> categorysecond);
  ...
  Future<String> sendDataToServer() async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'categoryfirst': widget.categoryfirst,
                ...
                ElevatedButton(
                  onPressed: () {
                    sendDataToServer().then((result) {
                      if (result == 'No match found between foodindetail and menu_name.') {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => FoodInfoPage(categorysecond, 'http://192.168.56.1:8080/api/foodInfo'),
                          ),
                        );
                      }
                    });
